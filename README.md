...
- **规则数据化**：**515** 条高价值清理规则以 TOML 声明，可 diff、可禁用、可 fixture 回归（含用户域 orphaned app data）

适合：想要更现代、更可脚本化、更偏「安全预览再执行」的日常清理与磁盘洞察。若你需要成熟全家桶（`purge` / `installer` / 真 sudo 删除 / 受保护组容器缓存完整对齐），请继续用 [Mole](https://github.com/tw93/Mole)。
...
**515** 条高价值规则以 TOML 声明，覆盖浏览器缓存、开发工具、应用残留与用户域 orphaned 数据等常见目标。
...
| 规则规模 | **515** 条高价值规则 | 全量数百条 `safe_clean` 目标 |
