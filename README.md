...
- **规则数据化**：**533** 条高价值清理规则以 TOML 声明，可 diff、可禁用、可 fixture 回归（含用户域 orphaned app data）
...
**533** 条高价值规则以 TOML 声明，覆盖浏览器缓存、开发工具、应用残留与用户域 orphaned 数据等常见目标。
...
| 成熟度 | **1.29.0**：本地快照报告（status/analyze）+ TM 失败中备份 + system.sh 主链；余项：桌面 Helper | 成熟、功能最全 |
