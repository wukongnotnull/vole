...
| **规则数据化** | **475** 条高价值清理规则以 TOML 声明，可 diff、可禁用、可 fixture 回归 |
...
| 规则规模 | **475** 条高价值规则（Phase 4c+ 持续扩展） | 全量数百条 `safe_clean` 目标 |
...
| Phase 4 `clean` | 可用；规则 **475** 条（含 guard cmdline + FCP / 剪映 generated） |
| Phase 5 `history` + 协议冻结 + 菜单 / 补全 | 可用；**v0.0.10** Developer ID + 公证（475 规则） |
