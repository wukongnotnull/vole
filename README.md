...
| 成熟度 | **1.41.0**：Batch6 兄弟路径 + QQ Music AS 缓存 + Chrome DevTools MCP / Antigravity browser Cache + optimize W2b③ + Zed npm + memory_pressure + uninstall 系统/Login/brew + Filo + DNS/mDNS + 本地快照 + TM 失败备份 + system.sh 主链；余项：Mole 广谱 `/Library` 边缘 / optimize spotlight* 等长尾 / 桌面 Helper | 成熟、功能最全 |
