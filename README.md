...
- **规则数据化**：**525** 条高价值清理规则以 TOML 声明，可 diff、可禁用、可 fixture 回归（含用户域 orphaned app data）
...
**525** 条高价值规则以 TOML 声明，覆盖浏览器缓存、开发工具、应用残留与用户域 orphaned 数据等常见目标。
...
| 规则规模 | **525** 条高价值规则 | 全量数百条 `safe_clean` 目标 |
