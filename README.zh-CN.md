...
当前已发布版本：**[v2.18.0](https://github.com/wukongnotnull/vole/releases/tag/v2.18.0)**（Developer ID 签名并经 Apple 公证）。Apple Silicon 与 Intel 均有对应安装包。
...
装好后直接运行 `vole`。若 brew 安装失败或版本对不上，请改用下方「下载安装包」。
...
示例（Apple Silicon / v2.18.0；请以 Release 页实际文件名为准）：
...
清理规则与安全思路受到 [Mole](https://github.com/tw93/Mole) 启发；Vole 是独立开源项目，不隶属于 Mole。  
