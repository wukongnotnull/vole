...
pub fn apply_plan(
    ...
    for (idx, entry) in plan.entries.iter().enumerate() {
        ...
        // 系统 DiagnosticReports：形状叶 → 年龄 ≥7d → allowlist → probe → sudo permanent。
        if entry.rule_id == DIAGNOSTIC_REPORTS_SYSTEM_RULE_ID {
            let fallback = NoPrivilege;
            let backend = ctx.privilege.unwrap_or(&fallback);
            if !entry
                .path
                .to_str()
                .is_some_and(is_system_diagnostic_report_leaf)
                || !entry.path.is_file()
                || !path_mtime_older_than_days(&entry.path, DIAGNOSTIC_REPORTS_SYSTEM_AGE_DAYS)
                || !path_allowed_for_privilege(&entry.path)
            {
                skipped += 1;
                if let Some(event) = &ctx.on_event {
        ...
        // `/private/var/log`：形状目标 → 文件 → 年龄 ≥7d → allowlist → probe → sudo permanent。
        if entry.rule_id == PRIVATE_VAR_LOG_RULE_ID {
            let fallback = NoPrivilege;
            let backend = ctx.privilege.unwrap_or(&fallback);
            if !entry
                .path
        ...
        // `/private/var/db/DiagnosticPipeline`：形状 → 文件 → ≥7d → allowlist → probe → sudo permanent。
        if entry.rule_id == PRIVATE_VAR_DB_DIAGNOSTIC_PIPELINE_RULE_ID {
            let fallback = NoPrivilege;
            let backend = ctx.privilege.unwrap_or(&fallback);
            if !entry
                .path
                .to_str()
                .is_some_and(is_private_var_db_diagnostic_pipeline_clean_target)
                || !entry.path.is_file()
                || !path_mtime_older_than_days(
                    &entry.path,
                    PRIVATE_VAR_DB_DIAGNOSTIC_PIPELINE_AGE_DAYS,
                )
                || !path_allowed_for_privilege(&entry.path)
            {
                skipped += 1;
        ...
        // `/private/var/db/powerlog`：形状 → 文件 → ≥7d → allowlist → probe → sudo permanent。
        if entry.rule_id == PRIVATE_VAR_DB_POWERLOG_RULE_ID {
            let fallback = NoPrivilege;
            let backend = ctx.privilege.unwrap_or(&fallback);
            if !entry
                .path
                .to_str()
                .is_some_and(is_private_var_db_powerlog_clean_target)
                || !entry.path.is_file()
                || !path_mtime_older_than_days(&entry.path, PRIVATE_VAR_DB_POWERLOG_AGE_DAYS)
                || !path_allowed_for_privilege(&entry.path)
            {
                skipped += 1;
                if let Some(event) = &ctx.on_event {
        ...
        // MemoryLimitViolations：形状 → 文件 → ≥30d → allowlist → probe → sudo permanent。
        if entry.rule_id == PRIVATE_VAR_DB_MEMORY_LIMIT_VIOLATIONS_RULE_ID {
            let fallback = NoPrivilege;
            let backend = ctx.privilege.unwrap_or(&fallback);
            if !entry
                .path
                .to_str()
                .is_some_and(is_private_var_db_memory_limit_violations_clean_target)
                || !entry.path.is_file()
                || !path_mtime_older_than_days(
                    &entry.path,
                    PRIVATE_VAR_DB_MEMORY_LIMIT_VIOLATIONS_AGE_DAYS,
        ...
        // private-tmp：形状（深度 1）→ 文件 → ≥7d → allowlist → probe → sudo permanent。
        if entry.rule_id == PRIVATE_TMP_RULE_ID {
            let fallback = NoPrivilege;
            let backend = ctx.privilege.unwrap_or(&fallback);
            if !entry.path.to_str().is_some_and(is_private_tmp_clean_target)
                || !entry.path.is_file()
                || !path_mtime_older_than_days(&entry.path, PRIVATE_TMP_AGE_DAYS)
                || !path_allowed_for_privilege(&entry.path)
            {
                skipped += 1;
                if let Some(event) = &ctx.on_event {
        ...
        // library-caches-temp：形状（≤5 + 扩展名）→ 文件 → 分龄 → allowlist → probe → sudo permanent。
        if entry.rule_id == LIBRARY_CACHES_TEMP_RULE_ID {
            let fallback = NoPrivilege;
            let backend = ctx.privilege.unwrap_or(&fallback);
            if !entry
                .path
                .to_str()
                .is_some_and(is_library_caches_temp_clean_target)
                || !entry.path.is_file()
                || !path_mtime_older_than_days(
                    &entry.path,
        ...
        // code-sign-clone：形状（*/X/*/*.code_sign_clone 目录）→ 非 EDR → allowlist → probe → sudo permanent。
        if entry.rule_id == CODE_SIGN_CLONE_RULE_ID {
            let fallback = NoPrivilege;
            let backend = ctx.privilege.unwrap_or(&fallback);
...
"#,
    ...
    #[test]
    fn apply_diagnostic_reports_system_records_remove_when_old() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::DIAGNOSTIC_REPORTS_SYSTEM_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("diag-sys-ok");
        let lib = root.join("Library");
        let leaf = lib.join("Logs/DiagnosticReports/App.crash");
        fs::create_dir_all(leaf.parent().unwrap()).unwrap();
        fs::write(&leaf, b"crash").unwrap();
        ...
        let plan = fresh_plan(vec![plan_entry(&leaf, DIAGNOSTIC_REPORTS_SYSTEM_RULE_ID)]);
        let protection = AppProtection::new();
        let deletion_log = DeletionLogger::with_path(root.join("deletions.log"));
        let mut oplog = OperationLogger::new("clean");
        let probe = crate::rules::FakeProcessProbe::default();
    ...
    #[test]
    fn apply_diagnostic_reports_system_skips_fresh_leaf() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::DIAGNOSTIC_REPORTS_SYSTEM_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("diag-sys-fresh");
        let lib = root.join("Library");
        let leaf = lib.join("Logs/DiagnosticReports/Fresh.crash");
        fs::create_dir_all(leaf.parent().unwrap()).unwrap();
        fs::write(&leaf, b"crash").unwrap();
        std::env::set_var("VOLE_TEST_SYSTEM_LIBRARY", &lib);
    ...
    #[test]
    fn apply_private_var_log_records_remove_when_old() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_VAR_LOG_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("pvl-ok");
        let lib = root.join("Library");
        ...
        let plan = fresh_plan(vec![plan_entry(&leaf, PRIVATE_VAR_LOG_RULE_ID)]);
        let protection = AppProtection::new();
        let deletion_log = DeletionLogger::with_path(root.join("deletions.log"));
        let mut oplog = OperationLogger::new("clean");
        let probe = crate::rules::FakeProcessProbe::default();
    ...
    #[test]
    fn apply_private_var_log_skips_fresh_leaf() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_VAR_LOG_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("pvl-fresh");
        let lib = root.join("Library");
        let leaf = root.join("private/var/log/fresh.log");
        fs::create_dir_all(leaf.parent().unwrap()).unwrap();
        fs::write(&leaf, b"log").unwrap();
        std::env::set_var("VOLE_TEST_SYSTEM_LIBRARY", &lib);
    ...
    #[test]
    fn apply_private_var_log_skips_three_tree_even_with_rule_id() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_VAR_LOG_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("pvl-threetree");
        let lib = root.join("Library");
    ...
    #[test]
    fn apply_private_var_db_diagnostic_pipeline_records_remove_when_old() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_VAR_DB_DIAGNOSTIC_PIPELINE_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("pvp-ok");
        let lib = root.join("Library");
        let leaf = root.join("private/var/db/DiagnosticPipeline/x.data");
        fs::create_dir_all(leaf.parent().unwrap()).unwrap();
        fs::write(&leaf, b"data").unwrap();
        let ancient = SystemTime::now() - Duration::from_secs(10 * 86400);
        filetime::set_file_mtime(&leaf, filetime::FileTime::from_system_time(ancient)).unwrap();
        ...
        let plan = fresh_plan(vec![plan_entry(
            &leaf,
            PRIVATE_VAR_DB_DIAGNOSTIC_PIPELINE_RULE_ID,
        )]);
        let protection = AppProtection::new();
        let deletion_log = DeletionLogger::with_path(root.join("deletions.log"));
        let mut oplog = OperationLogger::new("clean");
    ...
    #[test]
    fn apply_private_var_db_diagnostic_pipeline_skips_fresh() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_VAR_DB_DIAGNOSTIC_PIPELINE_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("pvp-fresh");
        let lib = root.join("Library");
        let leaf = root.join("private/var/db/DiagnosticPipeline/fresh.data");
        fs::create_dir_all(leaf.parent().unwrap()).unwrap();
        fs::write(&leaf, b"data").unwrap();
        std::env::set_var("VOLE_TEST_SYSTEM_LIBRARY", &lib);
    ...
    #[test]
    fn apply_private_var_db_diagnostic_pipeline_skips_three_tree_even_with_rule_id() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_VAR_DB_DIAGNOSTIC_PIPELINE_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("pvp-threetree");
        let lib = root.join("Library");
        let plist = lib.join("LaunchDaemons/com.example.plist");
        fs::create_dir_all(plist.parent().unwrap()).unwrap();
        fs::write(&plist, b"plist").unwrap();
    ...
    #[test]
    fn apply_private_var_db_powerlog_records_remove_when_old() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_VAR_DB_POWERLOG_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("pwr-ok");
        let lib = root.join("Library");
        ...
        let plan = fresh_plan(vec![plan_entry(&leaf, PRIVATE_VAR_DB_POWERLOG_RULE_ID)]);
        let protection = AppProtection::new();
        let deletion_log = DeletionLogger::with_path(root.join("deletions.log"));
        let mut oplog = OperationLogger::new("clean");
        let probe = crate::rules::FakeProcessProbe::default();
    ...
    #[test]
    fn apply_private_var_db_powerlog_skips_fresh() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_VAR_DB_POWERLOG_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("pwr-fresh");
        let lib = root.join("Library");
        let leaf = root.join("private/var/db/powerlog/fresh.data");
        fs::create_dir_all(leaf.parent().unwrap()).unwrap();
        fs::write(&leaf, b"data").unwrap();
        std::env::set_var("VOLE_TEST_SYSTEM_LIBRARY", &lib);
    ...
    #[test]
    fn apply_private_var_db_powerlog_skips_three_tree_even_with_rule_id() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_VAR_DB_POWERLOG_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("pwr-threetree");
        let lib = root.join("Library");
        let plist = lib.join("LaunchDaemons/com.example.plist");
        fs::create_dir_all(plist.parent().unwrap()).unwrap();
        fs::write(&plist, b"plist").unwrap();
        ...
        let plan = fresh_plan(vec![plan_entry(&plist, PRIVATE_VAR_DB_POWERLOG_RULE_ID)]);
        let protection = AppProtection::new();
        let deletion_log = DeletionLogger::with_path(root.join("deletions.log"));
        let mut oplog = OperationLogger::new("clean");
        let probe = crate::rules::FakeProcessProbe::default();
    ...
    #[test]
    fn apply_memory_limit_violations_records_remove_when_old() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_VAR_DB_MEMORY_LIMIT_VIOLATIONS_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("mlv-ok");
        let lib = root.join("Library");
    ...
    #[test]
    fn apply_memory_limit_violations_skips_mid_age() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_VAR_DB_MEMORY_LIMIT_VIOLATIONS_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("mlv-mid");
        let lib = root.join("Library");
        let leaf = root.join("private/var/db/reportmemoryexception/MemoryLimitViolations/mid.data");
        fs::create_dir_all(leaf.parent().unwrap()).unwrap();
        fs::write(&leaf, b"data").unwrap();
        // 15d：>7 但 <30 → must skip
        let mid = SystemTime::now() - Duration::from_secs(15 * 86400);
        filetime::set_file_mtime(&leaf, filetime::FileTime::from_system_time(mid)).unwrap();
        std::env::set_var("VOLE_TEST_SYSTEM_LIBRARY", &lib);
    ...
    #[test]
    fn apply_memory_limit_violations_skips_three_tree_even_with_rule_id() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_VAR_DB_MEMORY_LIMIT_VIOLATIONS_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("mlv-threetree");
        let lib = root.join("Library");
        let plist = lib.join("LaunchDaemons/com.example.plist");
        fs::create_dir_all(plist.parent().unwrap()).unwrap();
        fs::write(&plist, b"plist").unwrap();
    ...
    #[test]
    fn apply_private_tmp_removes_old_depth1() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_TMP_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("ptmp-ok");
        let lib = root.join("Library");
        ...
        let plan = fresh_plan(vec![plan_entry(&leaf, PRIVATE_TMP_RULE_ID)]);
        let protection = AppProtection::new();
        let deletion_log = DeletionLogger::with_path(root.join("deletions.log"));
        let mut oplog = OperationLogger::new("clean");
        let probe = crate::rules::FakeProcessProbe::default();
    ...
    #[test]
    fn apply_private_tmp_skips_fresh() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_TMP_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("ptmp-fresh");
        let lib = root.join("Library");
        let leaf = root.join("private/var/tmp/fresh.file");
        fs::create_dir_all(leaf.parent().unwrap()).unwrap();
        fs::write(&leaf, b"tmp").unwrap();
        std::env::set_var("VOLE_TEST_SYSTEM_LIBRARY", &lib);
    ...
    #[test]
    fn apply_private_tmp_skips_nested_even_with_rule_id() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_TMP_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("ptmp-deep");
        let lib = root.join("Library");
        let leaf = root.join("private/tmp/sub/nested.file");
        fs::create_dir_all(leaf.parent().unwrap()).unwrap();
        fs::write(&leaf, b"tmp").unwrap();
        let ancient = SystemTime::now() - Duration::from_secs(10 * 86400);
        filetime::set_file_mtime(&leaf, filetime::FileTime::from_system_time(ancient)).unwrap();
    ...
    #[test]
    fn apply_private_tmp_skips_three_tree_even_with_rule_id() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::PRIVATE_TMP_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("ptmp-threetree");
        let lib = root.join("Library");
        let plist = lib.join("LaunchDaemons/com.example.plist");
        fs::create_dir_all(plist.parent().unwrap()).unwrap();
        fs::write(&plist, b"plist").unwrap();
        ...
        let plan = fresh_plan(vec![plan_entry(&plist, PRIVATE_TMP_RULE_ID)]);
        let protection = AppProtection::new();
        let deletion_log = DeletionLogger::with_path(root.join("deletions.log"));
        let mut oplog = OperationLogger::new("clean");
        let probe = crate::rules::FakeProcessProbe::default();
    ...
    #[test]
    fn apply_library_caches_temp_removes_old_leaf() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::LIBRARY_CACHES_TEMP_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("lcaches-ok");
        let lib = root.join("Library");
        ...
        let plan = fresh_plan(vec![plan_entry(&leaf, LIBRARY_CACHES_TEMP_RULE_ID)]);
        let protection = AppProtection::new();
        let deletion_log = DeletionLogger::with_path(root.join("deletion.log"));
        let mut oplog = OperationLogger::new("clean");
        let probe = crate::rules::FakeProcessProbe::default();
        let orphan_deps = FakeOrphanDeps {
            spotlight: true,
    ...
    #[test]
    fn apply_library_caches_temp_skips_fresh() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::LIBRARY_CACHES_TEMP_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("lcaches-fresh");
        let lib = root.join("Library");
        let leaf = lib.join("Caches/com.apple.foo/a.tmp");
        fs::create_dir_all(leaf.parent().unwrap()).unwrap();
        fs::write(&leaf, b"tmp").unwrap();
        std::env::set_var("VOLE_TEST_SYSTEM_LIBRARY", &lib);
    ...
    #[test]
    fn apply_library_caches_temp_skips_wrong_ext_even_with_rule_id() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::LIBRARY_CACHES_TEMP_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("lcaches-ext");
        let lib = root.join("Library");
        let leaf = lib.join("Caches/foo.dat");
        fs::create_dir_all(leaf.parent().unwrap()).unwrap();
        fs::write(&leaf, b"dat").unwrap();
        let ancient = SystemTime::now() - Duration::from_secs(10 * 86400);
        filetime::set_file_mtime(&leaf, filetime::FileTime::from_system_time(ancient)).unwrap();
        std::env::set_var("VOLE_TEST_SYSTEM_LIBRARY", &lib);
    ...
    #[test]
    fn apply_library_caches_temp_skips_three_tree_even_with_rule_id() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::LIBRARY_CACHES_TEMP_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("lcaches-threetree");
        let lib = root.join("Library");
        let plist = lib.join("LaunchDaemons/com.example.plist");
        fs::create_dir_all(plist.parent().unwrap()).unwrap();
        fs::write(&plist, b"plist").unwrap();
    ...
    #[test]
    fn apply_code_sign_clone_removes_dir() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::CODE_SIGN_CLONE_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("csc-ok");
        let lib = root.join("Library");
        ...
        let plan = fresh_plan(vec![plan_entry(&dir, CODE_SIGN_CLONE_RULE_ID)]);
        let protection = AppProtection::new();
        let deletion_log = DeletionLogger::with_path(root.join("deletions.log"));
        let mut oplog = OperationLogger::new("clean");
        let probe = crate::rules::FakeProcessProbe::default();
    ...
    #[test]
    fn apply_code_sign_clone_skips_edr_even_with_rule_id() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::CODE_SIGN_CLONE_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("csc-edr");
        let lib = root.join("Library");
        let dir =
            root.join("private/var/folders/zz/uid/X/com.crowdstrike.falcon.App.code_sign_clone");
        fs::create_dir_all(&dir).unwrap();
        std::env::set_var("VOLE_TEST_SYSTEM_LIBRARY", &lib);
    ...
    #[test]
    fn apply_code_sign_clone_skips_wrong_segment_even_with_rule_id() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::CODE_SIGN_CLONE_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("csc-wrong");
        let lib = root.join("Library");
        let dir = root.join("private/var/folders/zz/uid/C/Foo.app.code_sign_clone");
        fs::create_dir_all(&dir).unwrap();
        std::env::set_var("VOLE_TEST_SYSTEM_LIBRARY", &lib);
    ...
    #[test]
    fn apply_code_sign_clone_skips_three_tree_even_with_rule_id() {
        use crate::orphan::FakeOrphanDeps;
        use crate::privilege::CODE_SIGN_CLONE_RULE_ID;
        use std::collections::HashSet;

        let _guard = test_env::lock();
        let root = scratch("csc-threetree");
        let lib = root.join("Library");
        let plist = lib.join("LaunchDaemons/com.example.plist");
        fs::create_dir_all(plist.parent().unwrap()).unwrap();
        fs::write(&plist, b"plist").unwrap();
