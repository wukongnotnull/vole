...
- [ ] Task 1: `validate_path_for_deletion` + `is_critical_deletion_path` + 测试（`core_safe_functions.bats` 子集）
- [ ] Task 2: `is_endpoint_security_cache_path` + 前缀数据
- [ ] Task 3: `should_protect_path` 数据化（TOML）+ 判定引擎
...
- [ ] Task 7: 规则 TOML schema + 策略 trait（glob、mtime、pgrep…）
...
- [ ] Task 12: `vole clean --plan --json-stream`
