...
- 日期：2026-08-08（修订：同日 W2a① brew cask / 1.33.0 合入后更新状态）
...
| W2a② login items | — | 仍后置（轨内下一刀） |
| W2a③ LaunchDaemons | — | 仍后置 |
| W2b② `memory_pressure_relief` 及更后 | — | 仍后置 |
| W2c Batch 6+ 续挑 | — | 可继续 |
| W3 | — | 永不做 / 延后 |
...
| ②（仍后置） | login items | 可删 / 审计 |
| ③（仍后置） | 系统 LaunchDaemons / `/Library` sudo 残留（复用 PrivilegeBackend / `sudo -n`） | 可删 + 需特权 |

- **触点**：uninstall plan/apply、protection
- **并行**：可与 W2b / W2c 并行；**不得**两 agent 同改 uninstall 核心路径；轨内按 ①→②→③ 发版；① 已合入后轨内下一刀为 ②。
...
6. **并行池续刀**（**推荐下一刀**；任选一轨开 PR；另轨可并进）：
   - **W2a② login items**（默认推荐；W2a 轨内下一刀），或
   - W2b② `memory_pressure_relief`（或轨内更后项；仍属后置择优），或
   - W2c Batch 6+ 再挑一条窄 `all` 规则
7. W2 其余按各轨内顺序（W2a③ LaunchDaemons 等）
8. **W3** 仅维持 coverage / README 诚实句，不排开发
...
- [x] W0/W1/W2a①/W2b①/W2c 首刀状态与 main（1.28.0 / 1.29.0 / 1.33.0 / 1.31.0 / 1.32.0）一致

下一步：从 §3 第 6 项（W2 并行池续刀；默认 **W2a② login items**）选刀 → 单独 design + implementation plan。
