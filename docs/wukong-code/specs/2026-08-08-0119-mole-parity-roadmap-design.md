...
- 日期：2026-08-08（修订：同日 W2c 续刀 `zed-npm-system-cache` / 1.37.0 合入后更新状态）
...
| **W2a/b/c** | W0/W1 已解除阻塞；三轨彼此可并行；**轨内串行发版**；**W2b①② / W2c 首刀+续刀 / W2a①②③ 已完成**；**当前推荐**续挑窄项 | 可删 / 需特权 action |
| **W3** | 不开发 | 永不做 / 延后 |
...
| W2b③ 及后置 | — | 仍后置（轨内下一刀 · **默认推荐**） |
| W2c Batch 6+ 再续挑 | — | 可继续（并列） |
| W3 | — | 永不做 / 延后 |
...
依据：[`../../findings/2026-07-v2-m2-optimize-spike.md`](../../findings/2026-07-v2-m2-optimize-spike.md)；`optimize/catalog.rs` 中 `in_m3: false`；设计 [`2026-08-08-0156-optimize-system-network-design.md`](2026-08-08-0156-optimize-system-network-design.md)、[`2026-08-08-1121-optimize-memory-pressure-design.md`](2026-08-08-1121-optimize-memory-pressure-design.md)
...
| ③（仍后置 · **默认推荐下一刀**） | `network_stack_optimize` / `disk_permissions_repair` / `periodic_maintenance` | 网络栈 / 权限 / periodic | 需特权 action |
...
| 按价值再续挑窄规则 | `dev.sh` / `app_caches` 其余纯 `all`（Batch 6+ 可继续） | 可删 |
| **不进本波必做** | `user.sh` 广域、盲扩 `custom` | 保持「继续用 Mole」 |

- **触点**：`data/rules/`、handlers、coverage 规则计数
- **并行**：与 W2a / W2b 并行；首刀与续刀已合入后 **Batch 6+ 仍可继续挑窄 `all` 规则**（单刀单 PR · 与 W2b③ 并列可选）。
- **设计 / 计划**：[`2026-08-08-1121-zed-npm-system-cache-design.md`](2026-08-08-1121-zed-npm-system-cache-design.md)；[`../plans/2026-08-08-1122-zed-npm-system-cache.md`](../plans/2026-08-08-1122-zed-npm-system-cache.md)
...
10. **并行池续刀**（**推荐下一刀写死**；可并轨）：
   - **W2b③** `network_stack_optimize` / `disk_permissions_repair` / `periodic_maintenance`（**默认推荐**；W2b 轨内下一刀），或
   - W2c Batch 6+ 再挑一条窄 `all` 规则（并列可选）
11. W2 其余按各轨内顺序（spotlight* 等后置择优）
12. **W3** 仅维持 coverage / README 诚实句，不排开发

## 4. 与既有文档关系

| 文档 | 关系 |
|---|---|
| [`2026-08-08-0025-mole-system-sh-backlog-design.md`](2026-08-08-0025-mole-system-sh-backlog-design.md) | **system 对照表仍权威**；本文件为其 **超集**；W0/W1/W2a①②③/W2b①②/W2c 首刀+续刀状态以本文件为准 |
...
- [x] W0/W1/W2a①②③/W2b①②/W2c 首刀+续刀状态与 main（1.28.0 / 1.29.0 / 1.33.0 / 1.34.0 / 1.35.0 / 1.31.0 / 1.36.0 / 1.32.0 / 1.37.0）一致

下一步：从 §3 第 10 项（**默认 W2b③** 网络栈/权限/periodic，或 W2c Batch 6+）选刀 → 单独 design + implementation plan。
