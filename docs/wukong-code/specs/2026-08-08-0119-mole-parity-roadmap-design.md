...
- 日期：2026-08-08（修订：同日 W2a② login items / 1.34.0 合入后更新状态）
...
| **W2a/b/c** | W0/W1 已解除阻塞；三轨彼此可并行；**轨内串行发版**；**W2b① / W2c 首刀 / W2a① / W2a② 已完成**；**当前推荐**续挑窄项 | 可删 / 需特权 action |
| **W3** | 不开发 | 永不做 / 延后 |
...
7. **并行池续刀**（**推荐下一刀**；任选一轨开 PR；另轨可并进）：
   - **W2a③ LaunchDaemons / `/Library` sudo**（默认推荐；W2a 轨内下一刀），或
   - W2b② `memory_pressure_relief`（或轨内更后项；仍属后置择优），或
   - W2c Batch 6+ 再挑一条窄 `all` 规则
8. W2 其余按各轨内顺序
9. **W3** 仅维持 coverage / README 诚实句，不排开发

## 4. 与既有文档关系

| 文档 | 关系 |
|---|---|
| [`2026-08-08-0025-mole-system-sh-backlog-design.md`](2026-08-08-0025-mole-system-sh-backlog-design.md) | **system 对照表仍权威**；本文件为其 **超集**；W0/W1/W2a①/W2a②/W2b①/W2c 首刀状态以本文件为准 |
...
- [x] W0/W1/W2a①/W2a②/W2b①/W2c 首刀状态与 main（1.28.0 / 1.29.0 / 1.33.0 / 1.34.0 / 1.31.0 / 1.32.0）一致

下一步：从 §3 第 7 项（W2 并行池续刀；默认 **W2a③ LaunchDaemons / `/Library` sudo**）选刀 → 单独 design + implementation plan。
