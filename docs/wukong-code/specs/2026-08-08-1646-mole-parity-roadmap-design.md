...
- 日期：2026-08-08（**本文件为当前权威**；接续 [`2026-08-08-0119-mole-parity-roadmap-design.md`](2026-08-08-0119-mole-parity-roadmap-design.md) 的 1.40.0 快照）
...
| W2c Batch 6 收口 siblings + QQ Music AS | **1.41.0**（规则 **538–540**） | ✅ 分支落地（待合入 main） |

曾短暂「暂停 Batch6 必做」；已取消并完成收口三刀：`antigravity-browser-siblings`、`chrome-devtools-mcp-siblings`、`qq-music-mac-as-caches`。

**默认下一刀：无。** optimize 后置长尾（spotlight* 等）**保持 coverage**；**W3 不开发**（仅记档）。
...
| W2c Batch 6 收口 siblings + QQ Music AS | **1.41.0** | ✅（规则 540；待合入） |
...
| 后置 | `spotlight_*` / `disk_verify` / `login_items_audit` / `shared_file_list_repair` | — | **不默认开刀** |

### 2.5 W2c — clean 规则 Batch 6+（必做已完成）

| 顺序 | 项 | 版本 | 形态 |
|---|---|---|---|
| 首刀 | `filo-production-cache`（534） | 1.32.0 / #71 | 可删 |
| 续刀 | `zed-npm-system-cache`（535） | 1.37.0 / #82 | 可删 |
...
3. **默认下一刀写死**：**无默认必做刀**；optimize 后置长尾保持 coverage；W3 不开发

## 4. 与既有文档关系

| 文档 | 关系 |
|---|---|
| [`2026-08-08-0119-mole-parity-roadmap-design.md`](2026-08-08-0119-mole-parity-roadmap-design.md) | **1.40.0 及更早快照**；进度以 **本文件** 为准 |
...
- [x] 相对 0119：取消 Batch6 暂停，记录 1.41.0 收口完成，默认下一刀为空
- [x] 与分支现状对齐：规则约 540、包版本 1.41.0、coverage 含 siblings / QQ Music AS

下一步：**无默认实现刀**。将 1.41.0 分支合入 main 后，本路线图近满配必做项关闭；optimize 长尾与 W3 仅记档。
