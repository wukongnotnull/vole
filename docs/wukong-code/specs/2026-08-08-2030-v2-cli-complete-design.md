...
`purge` → `hints` → `installer` → `touchid` → 自更新通道 `update`
...
| 项 | 结论 |
|---|---|
| 北极星 | CLI 做全（相对 Mole 家庭桶剩余子命令） |
| 推进方式 | 顺序里程碑；每命令专用 design → plan → 单 PR |
| 命令顺序 | `purge` → `hints` → `installer` → `touchid` → `update` |
...
6. 交互菜单与 shell 补全覆盖上述命令
7. README 明确「产品 v2 CLI 全家桶」；包版本线在 **`2.x`**，且至少完成首发 **`2.0.0`** 与收口时的最新 MINOR
8. coverage / findings 诚实记录长尾与桌面非目标
...
| 里程碑 | 内容 | 包版本 |
|---|---|---|
| **M4** | 五命令 Mole 库存与安全面 spike；划主路径 vs 长尾 | docs-only（可仍停在 1.46.x） |
| **M5** | `purge` + 测试 / 菜单 / 补全 | **`2.0.0`**（升 MAJOR，产品/包对齐） |
| **M6** | `hints` | **`2.1.0`** |
| **M7** | `installer` | **`2.2.0`** |
| **M8** | `touchid` | **`2.3.0`** |
| **M9** | 自更新 `update` | **`2.4.0`** |
| **收口** | findings + README「产品 v2 CLI 全家桶」 | 当时最新 `2.x` |

顺序：M4 → M5 / **2.0.0** → M6 → M7 → M8 → M9 → 收口。  
...
6. **许可证**：GPL-3.0-only
7. **Mole 钉版**：`third_party/mole-1.48.1`；升级钉版另议，不盲跟上游
